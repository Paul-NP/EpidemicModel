...
def test_init_error(value, name):
    with pytest.raises(FastFactorError):
        f = FastFactor(name, value)
...
def test_dynamic_error():
    with pytest.raises(FastFactorError):
        f = FastFactor('gamma', lambda t: 1 / (5 - t))
        arr = np.zeros((2, 2), dtype=np.float64)
        f.connect_matrix(arr, (0, 0))
        f.update(5)
...
def test_dynamic_from_dict(time, mode, result):
    func = FastFactor.func_by_keyframes({5: 0.05, 10: 0.1}, continuation_mode=mode)
    f = FastFactor('dyn', func)
    arr = np.zeros((2, 2), dtype=np.float64)
    f.connect_matrix(arr, (0, 0))
    f.update(time)
